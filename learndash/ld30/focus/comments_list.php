...
require_once get_theme_file_path() . '/App/Learndash/WalkerCommentLearnDash.php';
...
if (!function_exists('learndash_focus_mode_comments_list')) {
	...
	<div <?php comment_class('ld-focus-comment ptype-' . $post->post_type . ' ' . $role_classes . $learndash_avatar_class); ?>
		...
		<div class="ld-comment-avatar">
		    <?php echo wp_kses_post(get_avatar($comment->user_id)); ?>
			...
			<span class="ld-comment-author-name">
			    ...
			    if (!empty($userMeta)) {
				echo esc_html($userMeta['first_name'][0] . ' ' . $userMeta['last_name'][0]);
			    } else {
				echo $comment->comment_author;
			    }
			    if (!empty($role_name)) {
				echo esc_html(' (' . $userData->data->user_login . ')');
				//$role_name
			    }
			    ?>
			</span>
			<a class="ld-comment-permalink"
			   href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>">
			       <?php
			       printf(
				       // translators: placeholders: %1$s: Comment Date, %2$s: Comment Time
				       esc_html_x('%1$s at %2$s', 'placeholders: comment date, comment time', 'learndash'), '<span> ' . esc_html(get_comment_date()) . '</span>', '<span> ' . esc_html(get_comment_time()) . '</span>'
			       );
			       ?>
	 ...
	 */
	$args = apply_filters(
		'learndash_focus_mode_list_comments_args', array(
	    'style' => 'div',
	    'page' => get_query_var('cpage', 1),
	    'callback' => 'learndash_focus_mode_comments_list',
	    'walker' => new Walker_Comment_LearnDash()
		)
	);
