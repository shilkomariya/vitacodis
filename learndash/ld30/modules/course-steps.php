...
<div class="ld-content-actions">
    ...
    <?php } ?>
    <?php if ($learndash_current_post_type === "sfwd-lessons" && get_the_title() === 'Course Discussion') { ?>
        <div class="ld-content-action content-title">
	...
	<?php else : ?>
		...
		elseif ($learndash_next_step_id) :
	    	...
	    	<a class="<?php echo esc_attr($button_class); ?>" href="<?php echo esc_attr(learndash_get_step_permalink($learndash_next_step_id, $course_id)); ?>">
	    	    <span class="ld-text">1111<?php echo learndash_get_label_course_step_next(get_post_type($learndash_next_step_id)); ?></span>
			<?php if (is_rtl()) { ?>
			    <span class="ld-icon ld-icon-arrow-left"></span>
			<?php } else { ?>
			    <span class="ld-icon ld-icon-arrow-right"></span>
