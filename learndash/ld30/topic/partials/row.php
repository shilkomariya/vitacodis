...
$topic_class = apply_filters(
	'learndash-topic-row-class', 'ld-table-list-item-preview ld-primary-color-hover ld-topic-row ' .
	( $topic->completed ? 'learndash-complete' : 'learndash-incomplete' )
	. ' ' . ( $post_id == $topic_id ? 'ld-is-current-item' : '' ), $topic
);
...
<div class="ld-table-list-item" id="<?php echo esc_attr('ld-table-list-item-' . $topic->ID); ?>">
	...
	<span class="ld-topic-title">
	    <?php echo wp_kses_post(apply_filters('the_title', $topic->post_title, $topic->ID)); ?></span> <?php // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound ?>
	    <?php
	    /**
